...
class DashRPCClient:
    """
    A class representing a list of Dash testnet addresses.

    This class allows you to retrieve the balance, UTXO set, and transaction
    history of a Dash testnet address using a full node.
    ...
    def _get_transaction_history(self, txhash=None):
        ...
        try:
            ...
            for block_height in range(self.min_height, self.height + 1):
                if not block_hash:
                    break
                block = self._send_rpc_request("getblock", params=[block_hash, 2])[
                    "result"
                ]
